...
public class KdbndpSqlTranslatingExpressionVisitor : RelationalSqlTranslatingExpressionVisitor
{
    ...
    private readonly KdbndpJsonPocoTranslator _jsonPocoTranslator;
    private readonly KdbndpLTreeTranslator _ltreeTranslator;
    ...
    public KdbndpSqlTranslatingExpressionVisitor(
        RelationalSqlTranslatingExpressionVisitorDependencies dependencies,
        QueryCompilationContext queryCompilationContext,
        QueryableMethodTranslatingExpressionVisitor queryableMethodTranslatingExpressionVisitor)
        : base(dependencies, queryCompilationContext, queryableMethodTranslatingExpressionVisitor)
    {
        _sqlExpressionFactory = (KdbndpSqlExpressionFactory)dependencies.SqlExpressionFactory;
        _jsonPocoTranslator = ((KdbndpMemberTranslatorProvider)Dependencies.MemberTranslatorProvider).JsonPocoTranslator;
        _ltreeTranslator = ((KdbndpMethodCallTranslatorProvider)Dependencies.MethodCallTranslatorProvider).LTreeTranslator;
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        if (unaryExpression.NodeType == ExpressionType.ArrayLength)
        {
            ...
            if (sqlOperand!.Type == typeof(byte[]) &&
                (sqlOperand.TypeMapping is null || sqlOperand.TypeMapping is KdbndpByteArrayTypeMapping))
            {
                return _sqlExpressionFactory.Function(
                    "length",
                    new[] { sqlOperand },
...

        {
                ...
                arguments[1] is LambdaExpression wherePredicate)
            {
                if (wherePredicate.Body is MethodCallExpression wherePredicateMethodCall)
                {
                    ...
                    // Note: we also handle the this equality instead of Like, see KdbndpArrayMethodTranslator
    ...
    protected override Expression VisitBinary(BinaryExpression binaryExpression)
    {
        ...
        if (binaryExpression.NodeType == ExpressionType.ArrayIndex)
        {
            ...
            return
                ...
                // Other types should be subscriptable - but KingbaseES arrays are 1-based, so adjust the index.
    ...
    protected override Expression VisitNew(NewExpression newExpression)
    {
        ...
        if (newExpression.Constructor?.DeclaringType == typeof(DateTime))
        {
            ...
            if (newExpression.Constructor == DateTimeCtor2)
            {
                ...
                // DateTime's second component is an int, but KingbaseES's MAKE_TIMESTAMP accepts a double precision
            ...
            if (newExpression.Constructor == DateTimeCtor3 && newExpression.Arguments[6] is ConstantExpression { Value : DateTimeKind kind })
            {
                ...
                // DateTime's second component is an int, but KingbaseES's make_timestamp/make_timestamptz accepts a double precision.
                // Also chop off the last Kind argument which does not get sent to KingbaseES
