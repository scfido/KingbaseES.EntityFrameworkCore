...
public static class KdbndpIndexExtensions
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public static bool AreCompatibleForKdbndp(
        ...
        bool shouldThrow)
    {
                ...
                || !SameColumnNames(index, duplicateIndex, storeObject))
            {
                if (shouldThrow)
                {
                    throw new InvalidOperationException(
                        KdbndpStrings.DuplicateIndexIncludedMismatch(
        ...
        if (index.IsCreatedConcurrently() != duplicateIndex.IsCreatedConcurrently())
        {
            ...
            {
                throw new InvalidOperationException(
                    KdbndpStrings.DuplicateIndexConcurrentCreationMismatch(
                        index.Properties.Format(),
                        index.DeclaringEntityType.DisplayName(),
                        duplicateIndex.Properties.Format(),
                        duplicateIndex.DeclaringEntityType.DisplayName(),
        ...
        if (index.GetCollation() != duplicateIndex.GetCollation())
        {
            ...
            {
                throw new InvalidOperationException(
                    KdbndpStrings.DuplicateIndexCollationMismatch(
                        index.Properties.Format(),
                        index.DeclaringEntityType.DisplayName(),
                        duplicateIndex.Properties.Format(),
                        duplicateIndex.DeclaringEntityType.DisplayName(),
